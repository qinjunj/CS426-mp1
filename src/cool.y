...
%type <formals>  formal_list_nemp
%type <formal>   formal

%type <expression> expr
/* expr with an ending semicolon */
%type <expression> expr_semi 
%type <expression> trailing_let
%type <expressions> expr_list
/* for actual parameters in dispatch */
%type <expressions> actual_list 
%type <expressions> actual_list_nemp

%type <cases> case_list
%type <case_> case
%type <case_> case_semi
...
class_list
        ...
        | error ';'
          { if (VERBOSE_ERRORS) fprintf(stderr, "Skipping to next class due to a syntax error.\n");
            $$ = nil_Classes(); }
        | class_list error ';'
          { if (VERBOSE_ERRORS) fprintf(stderr, "Skipping to next class due to a syntax error.\n"); 
            $$ = $1; }
...
formal_list : /* empty formal list */
              { $$ = nil_Formals(); }
            | formal_list_nemp
              { $$ = $1; }
            ;

formal_list_nemp : formal /* signal formal */
                   { $$ = single_Formals($1); }
                 | formal_list_nemp ',' formal /* several formals */
...
actual_list : /* empty arguments */
              { $$ = nil_Expressions(); }
            | actual_list_nemp
              { $$ = $1; }
            ;

actual_list_nemp : actual_list_nemp ',' expr /* several arguments */
                   { $$ = append_Expressions($1, single_Expressions($3)); }
                 |  expr /* single argument */
                   { $$ = single_Expressions($1); }
                 ;

trailing_let : OBJECTID ':' TYPEID IN expr
               { $$ = let($1, $3, no_expr(), $5); }
             ...
             | OBJECTID ':' TYPEID ASSIGN expr ',' trailing_let
               { $$ = let($1, $3, $5, $7); }
             | error ',' trailing_let
               { if (VERBOSE_ERRORS) fprintf(stderr, "Skipping to next let variable due to a syntax error.\n"); 
                $$ = $3; }
             ;

case_list : case_list case_semi
            { $$ = append_Cases($1, single_Cases($2)); }
          | case_semi
            { $$ = single_Cases($1); }
          ;
/* in order to use left recursion */ 
case_semi : case ';'
            { $$ = $1; }
          ;
